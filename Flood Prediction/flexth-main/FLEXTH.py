...
input_dir  = Path(r'C:/Users/HP/Desktop/Presentation/flexth-main/input' )
output_dir = Path(r'C:/Users/HP/Desktop/Presentation/flexth-main/output' )
...
def flood_processing():
    ...
    flood                =   rasterio.open(  input_dir / 'flood.tif' ).read(1).astype('uint8')
    dtm                  =   rasterio.open(  input_dir / 'dtm.tif'   )
    ...
    if os.path.isfile(input_dir / 'exclusion.tif'):
        exclusion      =  rasterio.open(  input_dir / 'exclusion.tif' ).read(1).astype('uint8')
    else:
        exclusion      = np.full_like(flood, 0)
        
        
    if os.path.isfile(input_dir / 'obswater.tif'):
        obswater      =  rasterio.open(  input_dir / 'obswater.tif' ).read(1).astype('uint8')
    else:
        obswater      =  np.copy(flood)
    
    
    if os.path.isfile(input_dir / 'permanent_water.tif'):
        permanent_water      =  rasterio.open(  input_dir / 'permanent_water.tif' ).read(1).astype('uint8')
    else:
    ...
    ## AVERAGES THE DTM ALONG THE BORDER 
    ...
    for i in range (1,len(z_stats)):
        ...
        if  len_border < param_min_flood_size: 
            ...
            #plot
            if False:
                min_ =np.min(temp_dtm_border_flood_smooth) 
                max_ = np.max(temp_dtm_border_flood_smooth)
                import matplotlib.pyplot as plt
                plt.scatter(temp_position_flood[:,0], temp_position_flood[:,1], c=dtm_temp_statistic, cmap='viridis', marker='s')
                plt.clim(min_, max_)
                plt.scatter(temp_position_border[:,0], temp_position_border[:,1], c=temp_dtm_border_flood_smooth, cmap='viridis', marker='s')
                plt.clim(min_, max_)
    ...
    if os.path.isfile(input_dir / 'exclusion.tif'): 
        ...
        dim_flooded_area[dim_flooded_area<0]=0  #  prevents potential negtive values caused by overflow 
    ...
    WL[(WD==0) & (flood>0)  ] = dtm[(WD==0) & (flood>0) ]  + param_WD_star/100
    ...
    if param_output_map == "WD" or  param_output_map == "WL_WD" :
        ...
        with rasterio.open(
            output_dir / f'WD_{param_WL_estimation_method}_Smax_{param_threshold_slope}_Nmax_{param_max_number_neighbors}_a_{param_inverse_dist_exp}_Dmax_{param_max_propagation_distance}_A12_{param_distance_range}_gaps_{param_size_gaps_close}.tif',
            mode="w",
            driver="GTiff",
            compress='deflate',
            height=n_row,
    ...
    if param_output_map == "WL" or  param_output_map == "WL_WD" :
        ...
        with rasterio.open(
            output_dir / f'WL_{param_WL_estimation_method}_Smax_{param_threshold_slope}_Nmax_{param_max_number_neighbors}_a_{param_inverse_dist_exp}_Dmax_{param_max_propagation_distance}_A12_{param_distance_range}_gaps_{param_size_gaps_close}.tif',
            mode="w",
            driver="GTiff",
            compress='deflate',
            height=n_row,
